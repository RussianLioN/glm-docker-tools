...
# Architecture: FPATH + autoload in ~/.zshrc (automatic)
...
add_zsh_autoload() {
    ...
    local marker="# GLM_ALIASES_AUToload - DO NOT EDIT BELOW THIS LINE"
    local autoload_block="
# GLM Aliases - Auto-generated by setup-aliases.sh
# FPATH functions need explicit autoload
autoload -U glm glm-debug glm-no-del glm-help
"

    # Check if autoload block already exists
    if grep -q "GLM_ALIASES_AUToload" "$zshrc" 2>/dev/null; then
        log_info "✅ Autoload already configured in ~/.zshrc"
        return 0
    fi
    ...
    # Append autoload block
    echo "" >> "$zshrc"
    echo "$autoload_block" >> "$zshrc"

    log_success "✅ Added autoload to ~/.zshrc"
    return 0
}

# Install for Zsh (FPATH method + autoload in ~/.zshrc)
install_zsh() {
    log_info "Installing for Zsh (FPATH + autoload method)..."
...
# Remove autoload from ~/.zshrc
remove_zsh_autoload() {
    local zshrc="$HOME/.zshrc"

    # Check if autoload block exists
    if ! grep -q "GLM_ALIASES_AUToload" "$zshrc" 2>/dev/null; then
        log_info "ℹ️  No autoload block found in ~/.zshrc"
        return 0
    fi
    ...
    # Remove autoload block (from marker to end of GLM block)
    # This removes everything from the marker to the next empty line or EOF
    local temp_file=$(mktemp)
    awk '
    /^# GLM_ALIASES_AUToload/ { skip = 1; next }
    skip && /^$/ { skip = 0; next }
    !skip { print }
    ' "$zshrc" > "$temp_file"
    ...
    log_success "✅ Removed autoload from ~/.zshrc"
    return 0
}
...
uninstall_zsh() {
    ...
    if [[ -f "$target_func" ]]; then
        rm -f "$target_func"
        log_success "✅ Removed $target_func"
    fi

    if [[ -f "$target_comp" ]]; then
        rm -f "$target_comp"
        log_success "✅ Removed $target_comp"
    fi
...
uninstall_bash() {
    ...
    local target="$HOME/.local/share/bash-completion/completions/glm"
