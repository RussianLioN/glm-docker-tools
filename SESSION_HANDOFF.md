...
**P17: Автоматический запуск Docker Desktop** (📋 НОРМАЛЬНЫЙ приоритет)

**Проблема:** Скрипт завершается с ошибкой если Docker Desktop не запущен:
```
[ERROR] ❌ Docker daemon не запущен. Запустите Docker Desktop.
```

Наблюдение: Команда `docker` доступна (CLI установлен), но daemon не отвечает.

**Архитектура Docker на macOS:**
```
Docker CLI (всегда доступен) → Docker Desktop (нужно запускать) → Docker Daemon
```

**Ожидаемое поведение:**
- ✅ Автоматическое обнаружение статуса Docker Desktop
- ✅ Предложение автоматического запуска (`open -a Docker`)
...
1. Создать `scripts/docker-helper.sh` с функциями:
   ...
   - `start_docker_desktop()` - запуск Docker Desktop
   - `wait_for_docker_daemon()` - ожидание с прогрессом
2. Обновить `glm-launch.sh`:
   - Заменить `check_dependencies()` на умную проверку
   - Интерактивный запрос автозапуска
3. Поддержка macOS (`open -a Docker`) и Linux (`systemctl`)
...
| **P17** | Docker Auto-start | 📋 **НОРМАЛЬНЫЙ** | 📋 Запланировано |
...
6. **P17: Автоматический запуск Docker Desktop**
   - Автообнаружение статуса Docker Desktop
   - Предложение автозапуска (`open -a Docker`)
   - Ожидание готовности daemon с прогрессом
   - Команды `--docker-status` и `--docker-start`
...
- **[P17 Implementation](./docs/IMPLEMENTATION_PLAN.md#p17-автоматический-запуск-docker-desktop-📋-нормальная-важность)** - План автозапуска Docker
...
| 📋 **NORMAL** | **P17** | 📋 Planned | Автоматический запуск Docker Desktop |
